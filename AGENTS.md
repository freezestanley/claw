...
> 目的：每轮开发完成后的交付汇报保持统一、精简结构，避免冗长。

完成一轮开发并通过基本验证后，按以下固定结构汇报，每条只给标题 + 一句总结描述：

- **完成的任务描述**：逐条 `子任务标题 - 子任务总结描述`。
- **问题清单**：逐条 `问题标题 - 问题总结描述`；无则写“无”。
- **访问地址**：项目访问地址，用加粗高亮文字样式。
- **下一步**：明确是“继续修改”还是“验收通过 → 进入编译/发布流程”，并据 SO 追问用户是否还有修改。

约束：描述务必精简,不堆砌过程日志;不展开逐步推理。
